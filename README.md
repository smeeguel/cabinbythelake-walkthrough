...
The published page opens on a gate that says all of this and asks for an age
confirmation before it draws anything else.
...
| Path | |
|---|---|
| `index.html` | The app: one self-contained document, ~3 MB. The hint book, with the full walkthrough one press away on every entry; the age gate and disclaimer come first. Generated. |
| `atlas.html`, `book.html` | The two pages this used to be, now redirects to `index.html` that keep the `#t=` link. Generated. |
| `_tools/` | The pipeline that builds it, plus its verification harnesses. |
| `robots.txt` | Nothing here asks to be indexed; the page also carries `noindex`. |
...
The built pages are tracked, unusually for generated output — GitHub Pages
...
python _tools/make_app.py                           # -> index.html
```
...
python _tools/probe.py            # "0 findings" at every width
```

## QA console

`_qa/run_qa.bat` (or `python _tools/qa.py`) tests every target for
completion on the install in `_game/`, all targets or one, with a
plain-English verdict per target — completes and by what weekend, not played
as written by the game and where, cannot be granted as shipped and why (with
the script lines), or no route found. It needs only Python 3.9+; the report is one
self-contained `_qa/latest.html`. Start with `_qa/qa_instructions.md`;
`_docs/qa.md` has the details.
